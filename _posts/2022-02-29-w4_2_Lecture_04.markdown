...
In the first lecture, we learn that most of our work in Data Science revolves around combining advanced coding with computational statistics to summarize data, unravel relationship between variables, or to make predictions. Sometimes, these three types of analysis are not so obvious to distinguish. Hence, the purpose of this lecture is to draw a line between these types of analysis, in order to select the right kind of tools to solve a particular problem or to answer our research question. In Lecture 3, we review the use of descriptive statistics to answer questions such as: "What is the current state of affairs?"; "How often, how many, when?" An we also pointed out the use of correlation to answer "what is the association between two variables?". To show that two variables have an association sometime is not enough, in some cases we would like to know the determinants and the effects of variables. Imagine for intance the development of a vaccine or an important policy prescription. Obviosly, the research that backs-up these developemntes will impact the life of many people and a then a simple asociation between two variables is just not enough. In many cases we would like to move from showing a correlation between variables to find which variable is the cause or determinant of the other. This kind of research is the central quest of econometrics and Data Science and has a special place in empirical economics.
...
The study is weak in many regards. Firstly, he attempts to describe a causal mechanism that occurs at the micro level, namely, in the brain of Nobel Prize winners. However, his data only accounts for consumption at the country level. In other words, he is using a macro data at the country level to draw conclusions about the brain of researchers. The second problem is that he is not controlling for important confounding variables such as natural ability and the level of education of individuals. Also, there is no account for motivation and the number of weekly hours that researchers invest in their work. The lack of these controls, induces doubts in the estimation because they are important determinants of research productivity.

- Data

He uses observational data, and he does not solve the problem of self-selection. The problem of self-selection for this study is clear as it is impossible to observe
